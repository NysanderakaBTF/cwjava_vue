...
export default {
  ...
  created() {
      ...
      axios.get(`http://127.0.0.1:8000/api/user/${store.state.auth.user.username}`).then(
          value => this.userr = value.data
      )
      axios.get(`http://127.0.0.1:8000/api/ticket/u/${store.state.auth.userInfo.id}`).then(
